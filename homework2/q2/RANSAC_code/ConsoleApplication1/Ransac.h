...
public:
	 ...
	 *                       least squares fit.
	 * @param data The input from which the parameters will be estimated.parameters
	 ...
	 */
	static double compute(vector<S> &parameters, ParameterEstimator<T, S> *paramEstimator, vector<T> &data, short *&bestVotes, int numForEstimate, double desiredProbabilityForNoOutliers, double maximalOutlierPercentage);
   ...
   *
	 ...
	 *
	 */
	 static double compute(vector<S> &parameters, 
...
private:
	...
	static void computeAllChoices(ParameterEstimator<T,S> *paramEstimator, vector<T> &data, int numForEstimate,
								  short *bestVotes, short *curVotes, int &numVotesForBest, int startIndex, int n, int k, int arrIndex, int *arr);

	static void estimate(ParameterEstimator<T,S> *paramEstimator, vector<T> &data, int numForEstimate,
...
double Ransac<T, S>::compute(vector<S> &parameters, ParameterEstimator<T, S> *paramEstimator, vector<T> &data, short *&bestVotes, int numForEstimate, double desiredProbabilityForNoOutliers, double maximalOutlierPercentage)
{
	...
	vector<T *> leastSquaresEstimateData;
	vector<S> exactEstimateParameters;
	int i, j, k, l, numVotesForBest, numVotesForCur, maxIndex, numTries;
	bestVotes = new short[numDataObjects]; //one if data[i] agrees with the best model, otherwise zero
	...
	set<int *, SubSetIndexComparator > chosenSubSets(subSetIndexComparator);
	...
	for (i = 0; i<numTries; i++)
	{
		...
		for (l = 0; l<numForEstimate; l++)
		{
			...
			//			int selectedIndex = (int)(((float)(rand()%100)/(float)RAND_MAX)*maxIndex + 0.5);
		...
		pair< set<int *, SubSetIndexComparator >::iterator, bool > res = chosenSubSets.insert(curSubSetIndexes);
		...
		{  //this sub set already appeared, don't count this iteration
	...
	//release the memory
	set<int *, SubSetIndexComparator >::iterator it = chosenSubSets.begin();
	set<int *, SubSetIndexComparator >::iterator chosenSubSetsEnd = chosenSubSets.end();
...
template<class T, class S>
double Ransac<T, S>::compute(vector<S> &parameters,
	ParameterEstimator<T, S> *paramEstimator,
	vector<T> &data,
	int numForEstimate)
{
...
/*****************************************************************************/
template<class T, class S>
void Ransac<T, S>::computeAllChoices(ParameterEstimator<T, S> *paramEstimator, vector<T> &data, int numForEstimate,
	short *bestVotes, short *curVotes, int &numVotesForBest, int startIndex, int n, int k, int arrIndex, int *arr)
{
...
/*****************************************************************************/
template<class T, class S>
void Ransac<T, S>::estimate(ParameterEstimator<T, S> *paramEstimator, vector<T> &data, int numForEstimate,
	short *bestVotes, short *curVotes, int &numVotesForBest, int *arr)
{
	vector<T *> exactEstimateData;
	vector<S> exactEstimateParameters;
